...
## 2026-09-17 Android 卡片溢出修复

- Android 卡片的管理分组、编辑备注和删除表情改为通过“表情操作”菜单访问，避免卡片缩小后三个按钮与备注在同一行产生横向溢出；置顶入口保留。
- 卡片展示依据主题目标平台选择，界面测试在所有宿主系统上显式覆盖 Android 布局，桌面悬停复制测试也不再依赖宿主系统。
- 增加 132 和 152.6 宽度的窄卡片测试，检查长备注、菜单操作与置顶，并验证点击操作入口不会触发表情使用动作。
- 本机 `flutter analyze --no-pub`、完整 51 项 Flutter 测试及项目结构检查通过；界面测试显式使用 Android 主题。修复后的远端 Actions 尚未执行。

## 2026-09-17 构建改为手动触发

- GitHub Actions 构建流程仅保留 `workflow_dispatch`，推送代码和创建或更新 PR 不再自动触发构建。
- 手动构建继续执行 macOS、Windows、Android 的检查、测试、构建和产物上传；README 增加手动操作说明。
